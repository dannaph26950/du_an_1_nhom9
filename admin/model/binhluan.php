...
function listone_binhluan($id_binh_luan){
    $sql = "select * from binh_luan where id_binhluan='$id_binh_luan' ";
    $listone_binhluan = pdo_query($sql);
    return $listone_binhluan ;
}
//xoa
function delete_binhluan($id_binh_luan){
    $sql = "delete from binh_luan where id_binhluan='$id_binh_luan'";
    pdo_execute($sql);
    return $sql;
}
function update_binhluan($ten_bl,$id_khoa_hoc,$id_hoc_vien,$trang_thai,$id_binhluan)
{
    $sql = "UPDATE `binh_luan` SET `ten_bl`='$ten_bl',`id_khoa_hoc`='$id_khoa_hoc',`id_hoc_vien`='$id_hoc_vien' WHERE id_binhluan = '$id_binhluan'";
    pdo_execute($sql);
}
